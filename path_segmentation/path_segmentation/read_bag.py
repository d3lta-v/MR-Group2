...
class ObjectSubscriber(Node):
    ...
    def __init__(self):
        ...
        ### Create Twist message publisher for commanding vehicle position
    ...
    def listener_callback(self, msg):
        ...
        twist = Twist()
        if len(distances) > 0:
            self.get_logger().info(f'Bag(s) detected at {distances} ', throttle_duration_sec=0.1)
            if any(distance < self.desired_distance_from_obj for distance in distances):
                # Stop the vehicle when distance is within threshold
                twist.linear.x = 0.0
            else:
                # Move forward when bags are detected but not within threshold
                twist.linear.x = self.forward_speed
        else:
            # Stop the vehicle when no bags are detected. This might be problematic when the camera loses track of the bags.
            twist.linear.x = 0.0
